...
// does not.
{
  ...
  try {
    ...
    const { filesScanned, guardGaps } = scanRepo(dir, ["scripts"]);
    assert(filesScanned === 4, `all four scratch files are scanned (got ${filesScanned})`);
    const byFile = new Map(guardGaps.map((g) => [g.file, g.kind]));
    assert(
      guardGaps.length === 3 && !byFile.has("scripts/correct.mjs"),
      `only the three broken files red, the correct one does not (got: ${JSON.stringify(guardGaps)})`
    );
...
{
  const { filesScanned, findings, unparseable, guardGaps } = scanRepo(ROOT);
