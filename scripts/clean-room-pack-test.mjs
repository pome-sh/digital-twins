...
/**
 * The dashboard the twin just printed, fetched from the installed tarball.
 *
 * `assetPath()` only asks `existsSync`, so a page that shipped truncated, or a
 * bundle whose entry name drifted from `vite.config.ts`, passes every check
...
async function checkDashboard(twin, log) {
  ...
  if (page.status !== 200) fail(`twin ${twin}: dashboard / answered ${page.status}`);
  const html = await page.text();

  // Every asset the page names has to be one the server will actually serve.
  const referenced = [...html.matchAll(/(?:src|href)="\/?([^"]+)"/g)].map((m) => m[1]);
  if (referenced.length === 0) fail(`twin ${twin}: dashboard page references no assets`);
  for (const asset of referenced) {
    const res = await fetch(`${url.origin}/${asset}`);
    if (res.status !== 200) fail(`twin ${twin}: dashboard asset /${asset} answered ${res.status}`);
    const body = await res.text();
    if (body.length < 200) {
      fail(`twin ${twin}: dashboard asset /${asset} is ${body.length} bytes — truncated?`);
    }
  }

  // The data route is the one thing on this port that must not be open.
  const unkeyed = await fetch(`${url.origin}/api/snapshot`);
  if (unkeyed.status !== 401) {
    fail(`twin ${twin}: /api/snapshot without a key answered ${unkeyed.status}, expected 401`);
  }
  const keyed = await fetch(`${url.origin}/api/snapshot?k=${url.searchParams.get("k")}`);
  if (keyed.status !== 200) fail(`twin ${twin}: /api/snapshot with the printed key answered ${keyed.status}`);
  const body = await keyed.json();
  if (body?.twins?.[0]?.name !== twin) {
    fail(`twin ${twin}: snapshot named "${body?.twins?.[0]?.name}"`);
  }
  console.log(`  ✓ ${twin}: dashboard page + ${referenced.length} asset(s) served, /api/snapshot gated`);
}
...
for (const asset of [
  ...
  // The local dashboard's built page. Generated by `packages/dashboard`'s vite
  // build straight into `cli/assets/dashboard/` and shipped by `files:
  // ["assets"]` — gitignored, like `cli/dist`, so only a pack test can catch a
  // publish that forgot to build it.
  join("assets", "dashboard", "index.html"),
  join("assets", "dashboard", "app.js"),
  join("assets", "dashboard", "app.css"),
]) {
