...
// `minimal-viktor` and `minimal-viktor-langgraph` carry two copies of
// `model-routing.ts` and `telemetry.ts` for exactly this reason, and that pair —
// one integration, two frameworks — is the same relationship as this one.
//
// COMMENTS may differ between the two copies, and do: each explains itself to a
// reader of its own framework. BEHAVIOUR may not. If the two ever disagree about
// what an HTTP call does, one of them is wrong — reconcile, do not let it drift.
// `test/pome.test.ts` is the sibling's suite case for case, which is what makes
// the disagreement show up as a red rather than as a difference nobody read.
//
// Strip the comments from both files and the ONLY difference today is two
// declarations — `PomeRunEvidence.scores` / `.readings` and the two calls that
// fill them, where Braintrust's copy says `classifications` and renders columns.
// That is the framework's vocabulary reaching one field deep, and it is the
// boundary: anything else that shows up in that diff is drift.
//
...
import { readVerdicts, readingFeedback, scoreFeedback } from "./scoring.js";
import type { PomeVerdict, ReadingFeedback, ScoreFeedback } from "./scoring.js";
 ...
 * fail, and a LangSmith row of blank cells reads like a quiet afternoon.
 *
...
export interface PomeRunEvidence {
  ...
  scores: ScoreFeedback[];
  readings: ReadingFeedback[];
}
 ...
 * The framework adapter is `src/langsmith.ts` and the wiring is `src/index.ts`.
 * The moment this file names an eval platform, the claim that the Pome half is
 * the same in both examples stops being checkable by reading them side by side.
 */
