...
- `GET /repos/:owner/:repo/pulls/:number/diff` (fidelity `shape`) — F-729
  ...
  method", so the tool is inventoried `semantic` and the placeholder-patch gap
  is named in its Known-deviations cell above. The REST route keeps the `shape`
  tier and this deferral, because it is still one surface doing one thing.
...
Every one of the 66 registered routes is mounted from its declaration, so a route's method
and path have exactly one home. The body schemas that used to be module-private consts inside
`registerGitHubRoutes` — including one anonymous inline schema — are those declarations now,
and the eleven imperative `c.req.query` reads are gone with the helpers that made them
(`numberParam`, `requireParam`, `routeTail`, `contentPath`, `numberQuery`, `stateQuery`).
