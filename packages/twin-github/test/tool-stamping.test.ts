...
// `RecorderEvent.tool` on the GitHub twin, through BOTH transports.
...
/** One probe per assertable action, and each declares BOTH doors' calls rather than
 *  deriving one from the other — because the two doors do not carry an action's. */
type Probe = {
...
const PROBES: Record<string, (sha: string) => Probe> = {
  create_commit_status: (sha) => ({
    ...
    // Off the tool table since — GitHub's own MCP server registers no such name — so
    // this one reaches the tape through the unknown-tool path.
    servedOverMcp: false,
  }),
  ...
  // Unlike the two above — REST operations GitHub never made MCP tools — this one is a
  // tool the twin genuinely serves, so its MCP arguments have to be the.
  add_issue_comment: () => ({
    rest: {
