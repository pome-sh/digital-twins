...
| `GET /repos/:owner/:repo/pulls/:number` | hot | semantic | PR detail. Carries `stack` (F-1178), derived from the base chain (divergence #11). |
...
| `GET /repos/:owner/:repo/pulls/:number/comments` | hot | semantic | Inline review comments. The PR's conversation comments are `issues/:number/comments` instead (F-1151). |
...
| `PUT /repos/:owner/:repo/pulls/:number/update-branch` | hot | semantic | Semantic merge of base into head (F-735). |
