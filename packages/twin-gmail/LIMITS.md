...
| MCP `pageSize` | default `20`, max `50` | `search_threads` / `list_drafts`. **Not `list_labels`** — the adopted listing declares no page arguments on it, so every label comes back in one answer |
