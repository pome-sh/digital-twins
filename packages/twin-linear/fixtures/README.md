...
| Path | Role |
| --- | --- |
| `mcp-tools-list.raw.json` | The 22-tool MCP listing, verbatim. **This file IS the tool table** — `src/mcp.ts` derives it (F-1325) |
...
`https://mcp.linear.app/mcp` is OAuth-gated, so F-1326 recorded the upstream
golden as deferred (`fixtures/mcp-tools-list/linear.status.json`). Nothing here
has ever been compared to what Linear serves; re-sourcing it needs that token
and belongs to F-1329.
