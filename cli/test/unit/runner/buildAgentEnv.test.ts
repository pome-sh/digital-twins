...
// Regression: single-twin runs against an OLD cloud (no `per_twin` in the wire body)
// must stay BYTE-IDENTICAL to origin/main's hosted env, with ONE deliberate.
 ...
 *  acceptance bar for byte-identity over every key EXCEPT `POME_GITHUB_TOKEN`. */
...
/** Drop `POME_GITHUB_TOKEN` so the value-level byte-identity checks below can compare
 *  every OTHER key against `originMainEnv()` without re-asserting the defective. */
function withoutGithubToken(env: Record<string, string>): Record<string, string> {
  const { POME_GITHUB_TOKEN: _omitted, ...rest } = env;
  return rest;
}

/** `originMainEnv()`'s key order with `POME_GITHUB_TOKEN` put back in main's slot for
 *  it — directly after `POME_GITHUB_MCP_URL`. */
...
describe("buildAgentEnv — single-twin old-cloud byte-identity", () => {
  ...
  it("omitted per_twin: FULL env is byte-identical to origin/main (values + key order)", () => {
    ...
    // Main computes POME_GITHUB_TOKEN as `session.provider_credentials.github?.token
    // ?? session.agent_token`, which is the exact expression that hands the twin.
  ...
  it("empty per_twin ({}): FULL env is byte-identical to the omitted-per_twin case", () => {
    ...
    // Both sides come from the SAME buildAgentEnv code path (including whatever
    // POME_GITHUB_TOKEN it currently computes included) — this is a self-consistency.
