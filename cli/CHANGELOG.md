...
- **No CLI change.** Version-only bump, for the same reason 0.21.7 was: F-1472
  audited every `packages/*` npm script that constitutes a check, which
  touched `packages/twin-github/package.json` (two new script entries) —
  publish-relevant for the CLI under the gate's plain `packages/twin-` prefix
  match. Here it genuinely is not: tsup only inlines twin `src/`, and an npm
  `scripts` entry ships in no tarball. Bumped anyway rather than adding an
  exception list to the gate, per 0.21.7.
