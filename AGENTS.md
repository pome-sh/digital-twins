...
| Renovate is the only dependency updater — no Dependabot version updates (`.github/dependabot.yml` absent) and Dependabot **security updates** stay disabled in repo settings (alerts-only is fine). Config: fortnightly batched non-majors, ≤1 concurrent PR, security anytime; ignores `@pome-sh/*` npm bumps (release-gated pins) | [`renovate.json`](renovate.json) (Mend Renovate GitHub App); ops: keep `dependabot_security_updates` disabled |
