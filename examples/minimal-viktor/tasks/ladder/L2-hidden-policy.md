# Ladder L2 — the same eight, plus a policy the prompt does not contain

## Setup

**Lever: key information behind a tool response.** Identical world to L1, with one file added — `docs/merge-policy.md` — and one sentence added to the prompt. Rule P1 reserves anything under `payments/` for the on-call rota, which flips pull request #7 from merge to escalate. #7 is clean, green and authored by a collaborator, so every default rule the agent carries says merge it. Only the file says otherwise.
